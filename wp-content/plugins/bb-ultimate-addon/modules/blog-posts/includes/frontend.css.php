...
.fl-node-<?php echo $id; ?> .uabb-blog-posts .uabb-post-thumbnail img {
	width: 100%;
		...
		?>
.fl-node-<?php echo $id; ?> .uabb-blog-posts .uabb-post-thumbnail img {
		<?php echo ( 'left' == $settings->overall_alignment ) ? 'margin: 0;margin-right: auto;' : ( ( 'right' == $settings->overall_alignment ) ? 'margin: 0;margin-left: auto;' : '' ); ?>
}
		<?php
	}
}
...
.fl-node-<?php echo $id; ?> select.uabb-masonary-filters {
	...
	<?php
	echo ( uabb_theme_text_color( '' != $settings->taxonomy_filter_select_color ) ) ? 'color: ' . uabb_theme_text_color( $settings->taxonomy_filter_select_color ) . ';' : '';
...
.fl-node-<?php echo $id; ?> ul.uabb-masonary-filters > li {
	...
	echo ( '' != uabb_theme_text_color( $settings->masonary_text_color ) ) ? 'color: ' . uabb_theme_text_color( $settings->masonary_text_color ) . ';' : '';
		echo ( 'left' == $settings->masonary_overall_alignment ) ? 'margin-right: 10px;' : ( ( 'right' == $settings->masonary_overall_alignment ) ? 'margin-left: 10px;' : 'margin-right: 5px; margin-left: 5px;' );
	?>
